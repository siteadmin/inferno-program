...
require_relative './profile_definitions/head_occipital_frontal_circumference_percentile_definitions'

module Inferno
  module Sequence
    class USCore311HeadOccipitalFrontalCircumferencePercentileSequence < SequenceBase
      include Inferno::DataAbsentReasonChecker
      include Inferno::USCore311ProfileDefinitions

      title 'Pediatric Head Occipital-frontal Circumference Percentile Tests'

      description 'Verify support for the server capabilities required by the US Core Pediatric Head Occipital-frontal Circumference Percentile Profile.'

      details %(
        # Background

        The US Core #{title} sequence verifies that the system under test is able to provide correct responses
        for Observation queries.  These queries must contain resources conforming to US Core Pediatric Head Occipital-frontal Circumference Percentile Profile as specified
      ...
      test 'All must support elements are provided in the Observation resources returned.' do
        ...
        skip_if_not_found(resource_type: 'Observation', delayed: false)
        must_supports = USCore311HeadOccipitalFrontalCircumferencePercentileSequenceDefinitions::MUST_SUPPORTS
