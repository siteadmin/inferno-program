    ...
    class USCore311GoalSequence < SequenceBase
      ...
      def perform_search_with_status(reply, search_param, search_method: :get)
        ...
        ['proposed', 'planned', 'accepted', 'active', 'on-hold', 'completed', 'cancelled', 'entered-in-error', 'rejected'].each do |status_value|
          ...
          reply = get_resource_by_params(versioned_resource_class('Goal'), params_with_status, search_method: search_method)
      ...
      test :search_by_patient do
        ...
        patient_ids.each do |patient|
          ...
          assert search_by_post_resources.length == @goal_ary[patient].length, 'Expected search by POST to have same results as search by GET'
      ...
      test :validate_resources do
        ...
        bindings = USCore311GoalSequenceDefinitions::BINDINGS
