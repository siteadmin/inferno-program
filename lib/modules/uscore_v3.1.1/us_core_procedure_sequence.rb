    ...
    class USCore311ProcedureSequence < SequenceBase
      ...
      def perform_search_with_status(reply, search_param, search_method: :get)
        ...
        ['preparation,in-progress,not-done,on-hold,stopped,completed,entered-in-error,unknown'].each do |status_value|
          ...
          reply = get_resource_by_params(versioned_resource_class('Procedure'), params_with_status, search_method: search_method)
      ...
      test :search_by_patient do
        ...
        patient_ids.each do |patient|
          ...
          assert search_by_post_resources.length == @procedure_ary[patient].length, 'Expected search by POST to have same results as search by GET'
      ...
      test :validate_resources do
        ...
        bindings = USCore311ProcedureSequenceDefinitions::BINDINGS
